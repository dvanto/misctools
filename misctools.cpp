...
// дл€ 8..13 - PCINT0_vect, дл€ A0..A5 - PCINT1_vect, дл€ 0..7 - PCINT2_vect
void pciSetup(byte pin) {
	*digitalPinToPCMSK(pin) |= bit (digitalPinToPCMSKbit(pin));  // –азрешаем PCINT дл€ указанного пина
	PCIFR  |= bit (digitalPinToPCICRbit(pin)); // ќчищаем признак запроса прерывани€ дл€ соответствующей группы пинов
	PCICR  |= bit (digitalPinToPCICRbit(pin)); // –азрешаем PCINT дл€ соответствующей группы пинов
}
...
int LCD_misc::printf(const char buf[], ... )
{
	...
	int i = sprintf(_buf, buf, argptr ); // тут надо провер€ть
	print( _buf);
	Log.notice( _buf );
	return i;
}
